...
extern uint64_t HKeyLevel[MAX_PLY+1000];//ключи позиции
...
void SaveState(void)
{
 FILE *file=fopen("/usr/share/QNXChess/state.txt","wb");
 if (file==NULL) return;
 ...
 fwrite(HKeyLevel,sizeof(uint64_t),FirstLevel,file);
 fclose(file);
}
//----------------------------------------------------------------------------------------------------
//загрузить состояние
...
{
 Move_StartPos();
 FILE *file=fopen("/usr/share/QNXChess/state.txt","rb");
 if (file==NULL) return;
 ...
 fread(HKeyLevel,sizeof(uint64_t),FirstLevel,file);
 fclose(file);
}
//---------------------------------------------------------------------------------------------------
//инициализация игры
