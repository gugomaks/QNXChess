...
uint64_t HKeyLevel[MAX_MOVES];//ключи позиции
unsigned long FirstLevel=0;//начальный уровень
 
extern uint64_t ZobristKey[FIGURE_TYPE_PAWN+1][2][256];//[тип фигуры][цвет фигуры][позиция фигуры на доске 16x16]
extern uint64_t HKey;//хэш позиции
extern uint64_t ZobristKeyMove;//ключ смены хода
...
inline ENGINE_BOOL Move_IsCheck(COORD coord256)
{
 ...
 if (type==FIGURE_TYPE_QUEEN)
 {
  ...
  while(QueenMove[n]!=0)
  {
   ...
   while(Board[c256]==0) c256+=QueenMove[n];//пока можно ходить
   cell=Board[c256];
   if ((cell&(MASK_COLOR|MASK_TYPE))==mask_king) return(ENGINE_TRUE);//король противника
   n++;
  }
 ...
 if (type==FIGURE_TYPE_ROOK)
 {
  ...
  while(RookMove[n]!=0)
  {
   ...
   while(Board[c256]==0) c256+=RookMove[n];//пока можно ходить
   cell=Board[c256];
   if ((cell&(MASK_COLOR|MASK_TYPE))==mask_king) return(ENGINE_TRUE);//король противника
   n++;
  }
 ...
 if (type==FIGURE_TYPE_BISHOP)
 {
  ...
  while(BishopMove[n]!=0)
  {
   ...
   while(Board[c256]==0) c256+=BishopMove[n];//пока можно ходить
   cell=Board[c256];
   if ((cell&(MASK_COLOR|MASK_TYPE))==mask_king) return(ENGINE_TRUE);//король противника
   n++;
  }
...
inline void Move_CreateMove(COORD coord256,SMove *sMove_Ptr,long &move,SMove *sMove_EatPtr,long &move_eat,COORD coord256_passedpawn,SMove* &sMove_FirstPtr,SMove* &sMove_FirstEatPtr,SMoveKit *sMoveKitPtr)
{
 ...
 if (type==FIGURE_TYPE_KING)
 {
  ...
  while(KingMove[n]!=0)
  {
   COORD c256=coord256+KingMove[n];
   cell=Board[c256];
   if (cell==0) Move_AddMove(coord256,c256,FIGURE_TYPE_KING,MOVE_TYPE_SIMPLY,sMove_Ptr,move,sMove_FirstPtr,sMoveKitPtr);//клетка пустая
   else
   {
 ...
 if (type==FIGURE_TYPE_KNIGHT)
 {
  ...
  while(KnightMove[n]!=0)
  {
   COORD c256=coord256+KnightMove[n];    
   cell=Board[c256];
   if (cell==0)//можно ходить в эту клетку
   {     
  ...
  if (color==WHITE)//если это белая пешка
  {
   ...
   if (Board[coord256+16]==0)
   {
    ...
    if ((cell&MASK_IS_MOVED)==0)//пешка ещё не ходила и никого за первый допустимый ход не ела
    {
     ...
     if (Board[coord256+32]==0) Move_AddMove(coord256,coord256+32,FIGURE_TYPE_PAWN,MOVE_TYPE_SIMPLY,sMove_Ptr,move,sMove_FirstPtr,sMoveKitPtr);//клетка пустая
    }
   }
   //проверим, можно ли есть
   cell=Board[coord256-1+16];
