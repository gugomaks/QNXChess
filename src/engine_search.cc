...
extern uint64_t ZobristKeyNullMove;//ключ нулевого хода
extern uint64_t HKeyLevel[MAX_MOVES];//ключи позиции
 ...
 */
 ...
 while(depth<=MAX_LEVEL)
 {
  ...
  printf("info score cp %ld ",score);
  printf("depth %ld ",depth);
  printf("seldepth %ld ",MaxLevel);  
  printf("nodes %ld ",Positions);
  printf("pv ");
  for(long p=0;p<depth;p++)
  {
   ...
   unsigned char x1=coord64_1&7;
   unsigned char y1=coord64_1>>3;
   unsigned char x2=coord64_2&7;
   unsigned char y2=coord64_2>>3;
